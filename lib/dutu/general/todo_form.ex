defmodule Dutu.General.TodoForm do
  ...
  defp validate_start_date_required_if_daterange(changeset) do
    ...
    if get_field(changeset, :date_type) == @due_date_types.between and
         (!start_date or start_date == "") do
  ...
  def to_todo_params(%TodoForm{title: title, due_date: due_date, date_type: date_type})
      when date_type == @due_date_types.on do
    %{
      title: title,
      due_date: due_date,
      date_attrs: %{
        type: @due_date_types.on
      },
      approx_due_date: nil
    }
  end

  def to_todo_params(%TodoForm{
        title: title,
        start_date: start_date,
        end_date: end_date,
        date_type: date_type
      })
      when date_type == @due_date_types.between do
    %{
      title: title,
